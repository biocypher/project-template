from biocypher import BioCypher, FileDownload
from template_package.adapters.template_adapter import (
    TemplateAdapter,
    TemplateAdapterNodeType,
    TemplateAdapterEdgeType,
    TemplateAdapterProteinField,
    TemplateAdapterDiseaseField,
)

# Instantiate the BioCypher interface
# You can use `config/biocypher_config.yaml` to configure the framework or
# supply settings via parameters below
bc = BioCypher()

# Download and cache resources (change the directory in the options if needed)
urls = "https://file-examples.com/wp-content/storage/2017/02/file_example_CSV_5000.csv"
resource = FileDownload(
    name="Example resource",  # Name of the resource
    url_s=urls,  # URL to the resource(s)
    lifetime=7,  # seven days cache lifetime
)
paths = bc.download(resource)  # Downloads to '.cache' by default
print(f"Downloaded files to {[path for path in paths]}")
# You can use the list of paths returned to read the resource into your adapter.
# As this is only a demo, we will not do that here.

# Choose node types to include in the knowledge graph.
# These are defined in the adapter (`adapter.py`).
node_types = [
    TemplateAdapterNodeType.PROTEIN,
    TemplateAdapterNodeType.DISEASE,
]

# Choose protein adapter fields to include in the knowledge graph.
# These are defined in the adapter (`adapter.py`).
node_fields = [
    # Proteins
    TemplateAdapterProteinField.ID,
    TemplateAdapterProteinField.SEQUENCE,
    TemplateAdapterProteinField.DESCRIPTION,
    TemplateAdapterProteinField.TAXON,
    # Diseases
    TemplateAdapterDiseaseField.ID,
    TemplateAdapterDiseaseField.NAME,
    TemplateAdapterDiseaseField.DESCRIPTION,
]

edge_types = [
    TemplateAdapterEdgeType.PROTEIN_PROTEIN_INTERACTION,
    TemplateAdapterEdgeType.PROTEIN_DISEASE_ASSOCIATION,
]

# Create a protein adapter instance
adapter = TemplateAdapter(
    node_types=node_types,
    node_fields=node_fields,
    edge_types=edge_types,
    # we can leave edge fields empty, defaulting to all fields in the adapter
)


# Create a knowledge graph from the adapter
bc.write_nodes(adapter.get_nodes())
bc.write_edges(adapter.get_edges())

# Write admin import statement
bc.write_import_call()
# Write schema info to graph (important for BioChatter connectivity)
bc.write_schema_info(as_node=True)

# Print summary
bc.summary()
